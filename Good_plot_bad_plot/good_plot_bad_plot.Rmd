...
plot_usmap(data = average_accuracy_by_state, values = "avg_temp_accuracy", color = "white") +
  scale_fill_gradient(low = "lightgreen", high = "darkgreen", name = "Max Temp Accuracy") + 
  theme_minimal() + 
...
### Data manipulation
```{r}
outlook_counts <- weather_forecasts_HI %>%              ## call data
  count(forecast_outlook) %>%                           ## count amount of times forecast_outlook categories come up
  rename(word = forecast_outlook, freq = n)             ## rename columns to fit word cloud script, n is automatically created from count
```

### Plot
```{r}
wordcloud(words = outlook_counts$word,                                 ## call "words" aka forecast_outlook
          freq = outlook_counts$freq,                                  ## call frequency
          min.freq = 1,                                                ## Minimum frequency to include
          max.words = 100,                                             ## This limits the total words displayed although we only have a few
          random.order = FALSE,                                        ## arranges words by frequency
          colors = c("skyblue", "gold", "lightgrey"))                   ## Colors for sunny, rainy, partly cloudy that represent the colors in nature
```
# Actual bad plot
```{r}
  
forecast_counts <- weather_forecasts_HI %>%
  count(forecast_outlook) %>%   # Count occurrences of each forecast outlook
  rename(word = forecast_outlook, freq = n)  # R

words <- data.frame(
  word = c("Sunny", "Rainy", "Cloudy", "Partly Cloudy", "Snow", "Fog", "Windy"),
  freq = c(100, 80, 10000, 40, 30, 20, 10)
)

ggplot(forecast_counts, aes(x = reorder(word, freq), y = 1, fill = freq)) +
  geom_tile() +
  geom_text(aes(label = word), size = 4, color = "white") +
  scale_fill_gradient(low = "lightblue", high = "darkblue") +
  labs(title = "Weather Forecast Outlook Counts", x = "Forecast Outlook", y = "") + 
